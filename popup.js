...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const currentPatternsDisplay = document.getElementById('currentPatternsDisplay');
        ...
        for (const url of urls) {
            for (const patternString of patterns) {
                try {
                    ...
                    console.warn(`Invalid regex pattern ignored during filtering: ${patternString}`, e);
    ...
    function updatePopupUI() {
        chrome.runtime.sendMessage({ action: 'getRecordedUrls' }, (response) => {
            if (response) {
                ...
                const filteredUrls = filterUrlsByPatterns(allRecordedUrls, currentPatterns);
    ...
    setPatternsButton.addEventListener('click', () => {
        ...
        const patterns = patternInput.value.split('\n').map(p => p.trim()).filter(p => p !== '');

        if (patterns.length > 0) {
            chrome.runtime.sendMessage({ action: 'setTargetPatterns', patterns: patterns }, (response) => {
                if (response.success) {
                    showMessage(`Monitoring set for ${response.patterns.length} patterns.`, 'success');
            ...
            chrome.runtime.sendMessage({ action: 'setTargetPatterns', patterns: [] }, (response) => {
                if (response.success) {
