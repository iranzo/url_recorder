...
chrome.runtime.onInstalled.addListener(() => {
  chrome.storage.local.get(['targetPatterns', 'recordedUrls', 'isDebugMode'], (result) => {
    if (result.targetPatterns === undefined) {
      chrome.storage.local.set({ targetPatterns: [] });
      if (DEBUG_MODE) console.log("Background: Initialized targetPatterns to empty array.");
    }
    if (result.recordedUrls === undefined) {
      chrome.storage.local.set({ recordedUrls: [] });
      if (DEBUG_MODE) console.log("Background: Initialized recordedUrls to empty array.");
    }
    if (result.isDebugMode === undefined) {
      chrome.storage.local.set({ isDebugMode: false }); // Default to false
      if (DEBUG_MODE) console.log("Background: Initialized isDebugMode to false.");
    } else {
      DEBUG_MODE = result.isDebugMode; // Set initial DEBUG_MODE from storage
      if (DEBUG_MODE) console.log("Background: Loaded initial DEBUG_MODE from storage:", DEBUG_MODE);
    }
  });
});
...
function matchesAnyPattern(url, patterns) {
  if (!url || !patterns || patterns.length === 0) {
    if (DEBUG_MODE) console.log("matchesAnyPattern: No patterns set or URL invalid, returning false.");
    return false;
  }
  for (const patternString of patterns) {
    try {
      ...
      if (regex.test(url)) {
        if (DEBUG_MODE) console.log(`matchesAnyPattern: URL "${url}" matched pattern "${patternString}".`);
        return true;
      }
    } catch (e) {
      if (DEBUG_MODE) console.warn(`matchesAnyPattern: Invalid regex pattern ignored: "${patternString}"`, e);
    }
  }
  if (DEBUG_MODE) console.log(`matchesAnyPattern: URL "${url}" did not match any active pattern.`);
  return false;
}
    ...
    if (matchesAnyPattern(url, targetPatterns)) {
      if (!recordedUrls.includes(url)) {
        ...
        chrome.storage.local.set({ recordedUrls: recordedUrls }, () => {
          if (DEBUG_MODE) console.log(`addUrlToStorage: Successfully recorded URL: "${url}". Total URLs: ${recordedUrls.length}`);
        });
      } else {
        if (DEBUG_MODE) console.log(`addUrlToStorage: URL "${url}" is already recorded.`);
      }
    } else {
      if (DEBUG_MODE) console.log(`addUrlToStorage: URL "${url}" did not match any active pattern, not recording.`);
    }
  });
}
...
chrome.webNavigation.onBeforeNavigate.addListener((details) => {
  ...
  if (details.frameId === 0 && (details.url.startsWith('http://') || details.url.startsWith('https://'))) {
    if (DEBUG_MODE) console.log(`WebNavigation: onBeforeNavigate triggered for: "${details.url}"`);
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'setTargetPatterns') {
    const newTargetPatterns = Array.isArray(request.patterns) ? request.patterns.filter(p => typeof p === 'string' && p.trim() !== '') : [];
    chrome.storage.local.set({ targetPatterns: newTargetPatterns }, () => {
      if (DEBUG_MODE) console.log(`Background: Set new target patterns:`, newTargetPatterns);
    ...
    chrome.storage.local.get(['targetPatterns', 'recordedUrls'], (result) => {
      ...
      if (DEBUG_MODE) console.log(`Background: Sending recorded URLs (${recordedUrls.length}) and patterns (${targetPatterns.length}) to popup.`);
    ...
    if (Array.isArray(request.urls) && request.urls.length > 0) {
      if (DEBUG_MODE) console.log(`Background: Received ${request.urls.length} URLs from content script.`);
